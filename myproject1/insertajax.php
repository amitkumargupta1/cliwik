...
$name=$fetch1['name'];
mysql_query("insert into comment value('$msgid','$textcontent','$email','$emailpic','','$name','$date','$time')");
$query2=mysql_query("select * from comment where emailid='$z'");
$fetch2=mysql_fetch_array($query2);
$picname2=$fetch2['emailidpic'];
$msg_id=$fetch2['commentid'];
$query3=mysql_query("select * from comment where comment='$textcontent' && date='$date' && time='$time' && emailid='$email'");
$fetch3=mysql_fetch_array($query3);
$msg_id1=$fetch3['commentid'];
$name2=$fetch3['name'];
mysql_query("insert into notification2 value('','$msg_id1','$msgid','$email','$emailpic','$name','$date','$time','comment')");
$notification_query=mysql_query("select * from notification2 where commentid='$msg_id1'");
...
<div class="stbody"> 
<div class="stimg"><form method="post" action="profile1.php"><input type="hidden" name="<?php echo $msg_id1; ?>">
