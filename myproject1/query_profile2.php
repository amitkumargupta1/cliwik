...
<?php
include('config.php');
$q=mysql_query("select commentid from querycomment order by commentid desc limit 1;");
$fetch=mysql_fetch_array($q);
for($c=1;$c<=$fetch['commentid'];$c++)
{
if(isset($_POST[$c]))
{
$q=mysql_query("select emailid from querycomment where commentid='$c'");
$fetch=mysql_fetch_array($q);
$z1= $fetch['emailid'];
break;
}
...
$sql = mysql_query("select * from querydata where email='$z1' order by id desc limit 5");
...
$like_counter="$like_count1 like and you supported";
...
if($unlike_counter==0)
$unlike_counter="you unsuppported";
else
...
<?php echo $wall_imaging; ?><h1><?php echo $link_txt; ?></h1><br><?php echo $video; ?>
